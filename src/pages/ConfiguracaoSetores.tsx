...
import { CadastrarSetorModal } from '@/components/modals/CadastrarSetorModal';
...
import { useDepartments, useToggleDepartmentStatus, useCreateDepartment, useUpdateDepartment } from '@/hooks/useDepartments';
import { useDepartmentAuditLog } from '@/hooks/useDepartmentAuditLog';
...
const ConfiguracaoSetores = () => {
  ...
  const [selectedSetorHistorico, setSelectedSetorHistorico] = useState<Department | null>(null);

  // Hook para modais padronizados
  const modalStates = useStandardModals();

  // Hook para buscar histórico do setor selecionado
  const { data: departmentAuditLogs = [], refetch: refetchAuditLogs } = useDepartmentAuditLog(
    selectedSetorHistorico?.id || null
  );

  // Buscar setores do Supabase
  const { data: departments, isLoading: departmentsLoading, refetch } = useDepartments();
  ...
  const sortedData = useMemo(() => {
    ...
    if (searchValue.trim()) {
      filtered = filtered.filter(item => {
        return columns.some(column => {
          let value = item[column.key as keyof Department];
          
          if (column.key === 'isDeleted') {
            value = value ? 'Inativo' : 'Ativo';
          }
    ...
    if (sortColumn) {
      filtered = filtered.sort((a, b) => {
        let aValue = a[sortColumn as keyof Department];
        let bValue = b[sortColumn as keyof Department];
  ...
  const handleToggleStatus = (item: Department) => {
    ...
    if (isActive) {
      modalStates.showInactivateConfirmation(
        'Deseja inativar o setor?',
        'Ele não estará mais visível fora das configurações.'
      );
      (window as any).tempSelectedItem = item;
    } else {
  ...
  const handleConfirmToggleStatus = async () => {
    const item = (window as any).tempSelectedItem;
    if (!item) return;
    ...
    try {
      ...
      modalStates.hideProcessingModal();
    } finally {
      (window as any).tempSelectedItem = null;
    }
  };

  const handleSaveSetor = async (formData: any) => {
    ...
    try {
      if (isEditing && editingItem?.id) {
        await updateDepartmentMutation.mutateAsync({
          ...
          isStockroom: formData.isStockroom,
        });
  ...
  const handleHistorico = (setor: any) => {
    setSelectedSetorHistorico(setor);
    setHistoricoModalOpen(true);
  };

  // Função para obter ações baseadas no item
  const getActionsForItem = (item: Department) => [
    {
  ...
  React.useEffect(() => {
    if (historicoModalOpen && selectedSetorHistorico) {
      refetchAuditLogs();
    }
  }, [historicoModalOpen, selectedSetorHistorico, refetchAuditLogs]);
    ...
    <>
      ...
      <div className="flex w-full h-[calc(100vh-8rem)] min-h-0">
        <div className="flex-1 min-w-0">
          <DataGallery
            ...
            getStatusColor={(item: Department) => {
      ...
      <CadastrarSetorModal
        ...
        editData={editingItem ? {
          ...
          agencyId: editingItem.agencyId,
          isStockroom: editingItem.isStockroom
        } : undefined}
        isEditing={isEditing}
      />
      ...
      <HistoricoModal
        ...
        title={`Histórico - ${selectedSetorHistorico?.name || ''}`}
        actions={formatAuditLogsToUserActions(departmentAuditLogs)}
      />
    </>
  );
};

export default ConfiguracaoSetores;
