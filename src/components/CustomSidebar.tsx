...
export function CustomSidebar() {
  ...
  return (
      <aside
        className={`
          fixed left-0 top-12 h-[calc(100vh-48px)] bg-white border-r border-border
          ...
          <ScrollArea className="flex-1" viewportRef={scrollRef}>
            <div>
              {navigationSections.map((section, index) => (
                <div key={index} className="relative w-full min-w-0 flex-col p-2">
                  <div className={`
                    ...
                    ${isCollapsed ? 'px-2 !opacity-100 !-mt-0' : 'justify-start'}
                  `}>
                    {isCollapsed ? (
                  ...
                  <div className="mt-1">
                    <ul className="flex w-full min-w-0 flex-col gap-1 space-y-1">
                      {section.items.map((item) => (
                        <li key={item.url} className="group/menu-item relative">
                          {isCollapsed ? (
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <div className={`
                                  group h-9 rounded-md transition-all duration-200
                                  px-0 justify-center
                                  ${isActive(item.url) 
                                    ? 'bg-light-black text-primary' 
                                  ...
                                  >
                                     <item.icon 
                                       className={`
                                         ...
                                         ${isActive(item.url) 
                                           ? 'text-main-primary' 
                                           : 'text-second-dark group-hover:text-main-primary'
                                         }
                                       `}
                                     />
                                  </NavLink>
                            ...
                            <div className={`
                              group h-9 px-4 py-1.5 rounded-md transition-all duration-200
                              ${isActive(item.url) 
                                ? 'bg-light-black text-primary' 
                                : 'hover:bg-light-black hover:text-primary'
                              }
