...
export interface FormSelectFieldWithLabelProps {
  label: string;
  value?: string;
...
const FormSelectFieldWithLabel = ({ 
  label,
  value, 
  onValueChange, 
  placeholder, 
  ...
  return (
    <div className={cn("space-y-1", className)}>
      ...
      {error && typeof error === 'string' && (
        <p className="text-xs text-main-danger">{error}</p>
      )}
    </div>
  )
}
FormSelectFieldWithLabel.displayName = "FormSelectFieldWithLabel"

export { FormSelectFieldWithLabel }