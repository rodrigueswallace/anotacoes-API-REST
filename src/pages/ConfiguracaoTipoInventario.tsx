...
import { CadastrarTipoInventarioModal } from '@/components/modals/CadastrarTipoInventarioModal';
...
const ConfiguracaoTipoInventario = () => {
  ...
  const breadcrumbPath = [
    ...
    { label: "Tipo de inventário", current: true }
  ];
  ...
  const columns = [
    { key: 'nome', title: 'Nome do tipo', sortable: true, width: 'auto' },
    { key: 'frequencia', title: 'Frequência', sortable: true, width: 'auto' },
    { 
      key: 'exige_comissao', 
      title: 'Exige Comissão', 
      sortable: true, 
      width: 'auto',
  ...
  const sortedData = useMemo(() => {
    ...
    if (searchValue.trim()) {
      filtered = filtered.filter(item => {
        return columns.some(column => {
          ...
          // Tratamento especial para campos booleanos e status
          if (column.key === 'exige_comissao') {
            value = value ? 'Sim' : 'Não';
          } else if (column.key === 'status') {
            value = value === 'Ativo' ? 'Ativo' : 'Inativo';
          }
    ...
    return filtered;
  }, [inventoryTypes, sortColumn, sortDirection, searchValue, columns]);
  ...
  const handleToggleStatus = (item: any) => {
    ...
    const action = item.status === 'Ativo' ? 'inativar' : 'reativar';
  ...
  const handleConfirmToggleStatus = async () => {
    if (selectedItem) {
      try {
        ...
        modalStates.showProcessingModal('Processando alteração...');
        
        await toggleInventoryTypeStatus(selectedItem.id);
  ...
  // Função para adicionar novo tipo
  const handleAddNew = () => {
    setEditingItem(null);
    setIsEditing(false);
    setModalOpen(true);
  ...
  const handleSave = async (formData: any) => {
    ...
    try {
      if (isEditing && editingItem) {
        await updateInventoryType({ id: editingItem.id, data: formData });
        modalStates.completeEditFlow();
      } else {
        await createInventoryType(formData);
        modalStates.completeCreateFlow();
      }
    } catch (error) {
      console.error('Erro ao salvar tipo de inventário:', error);
      modalStates.hideAllModals();
    }
  };
      ...
      <div className="flex w-full h-[calc(100vh-8rem)] min-h-0">
        <div className="flex-1 min-w-0">
          <DataGallery
            ...
            searchPlaceholder="Pesquisar tipos de inventário..."
...
export default ConfiguracaoTipoInventario;
