...
import { CadastrarMetodoAquisicaoModal } from '@/components/modals/CadastrarMetodoAquisicaoModal';
...
import { useAcquisitionMethodAuditLog } from '@/hooks/useAcquisitionMethodAuditLog';
import { useAcquisitionMethodRequiredDocumentAuditLog } from '@/hooks/useAcquisitionMethodRequiredDocumentAuditLog';
import { getHistoryIconByActionType } from '@/lib/history-icons';
import { format } from 'date-fns';
import { ptBR } from 'date-fns/locale';
import { HistoricoModalWithTabs, HistoricoTab } from '@/components/modals/HistoricoModalWithTabs';

const ConfiguracaoMetodoAquisicao = () => {
  ...
  // Hooks de auditoria para histórico
  const { 
    data: methodAuditLogs = [], 
    refetch: refetchMethodAuditLogs 
  } = useAcquisitionMethodAuditLog(
  ...
  } = useAcquisitionMethodRequiredDocumentAuditLog(
    selectedItemHistorico ? parseInt(selectedItemHistorico.id) : null
  );
  ...
  const breadcrumbPath = [
    ...
    { label: "Métodos de aquisição", current: true }
  ];
  ...
  const columns = [
    { key: 'nome', title: 'Nome do método', sortable: true, width: 'auto' },
    { key: 'codigo_interno', title: 'Código interno', sortable: true, width: 'auto' },
    { 
      key: 'exige_documento', 
      title: 'Exige documento em anexo', 
      sortable: true, 
      width: 'auto',
      render: (value: boolean) => (
        <Badge 
          ...
          className={`bg-transparent font-normal border font-sora text-xs hover:bg-transparent ${
            value ? 'border-main-info text-main-info' : 'border-main-warning text-main-warning'
          }`}
        >
          {value ? 'Sim' : 'Não'}
        </Badge>
    ...
    { 
      ...
      render: (value: string) => (
        <Badge 
          variant="outline" 
          className={`bg-transparent font-normal border font-sora text-xs hover:bg-transparent ${
            value === 'Ativo' ? 'border-main-success text-main-success' : 'border-main-danger text-main-danger'
          }`}
        >
          {value}
        </Badge>
  ...
  const sortedData = useMemo(() => {
    ...
    if (searchValue.trim()) {
      filtered = filtered.filter(item => {
        return columns.some(column => {
          ...
          // Tratamento especial para campos booleanos e renderizados
          if (column.key === 'exige_documento') {
            value = value ? 'Sim' : 'Não';
          } else if (column.key === 'status') {
            // Incluir status renderizado na busca
            value = item.status; // Já vem como "Ativo" ou "Inativo"
          }
          
          return containsNormalized(String(value), searchValue);
        });
      });
    }
    ...
    return filtered;
  }, [methods, sortColumn, sortDirection, searchValue, columns]);
  ...
  const handleConfirmToggleStatus = async () => {
    if (selectedItem) {
      ...
      try {
        await toggleAcquisitionMethodStatus(selectedItem.id);
  ...
  // Função para adicionar novo método
  const handleAddNew = () => {
    setEditingItem(null);
    setIsEditing(false);
    setModalOpen(true);
  ...
  // Função para preparar abas do histórico
  const prepareHistoryTabs = (): HistoricoTab[] => {
    const tabs: HistoricoTab[] = [];

    // Aba do método principal
    tabs.push({
      key: 'method',
      label: 'Método de Aquisição',
      actions: formatAuditLogsToUserActions(methodAuditLogs),
    });

    // Aba de documentos obrigatórios
    if (documentsAuditLogs.length > 0) {
      tabs.push({
        key: 'documents',
        label: 'Documentos Obrigatórios',
        actions: formatAuditLogsToUserActions(documentsAuditLogs),
      });
    }
  ...
  useEffect(() => {
    if (historicoModalOpen && selectedItemHistorico) {
      refetchMethodAuditLogs();
      refetchDocumentsAuditLogs();
    }
  }, [historicoModalOpen, selectedItemHistorico, refetchMethodAuditLogs, refetchDocumentsAuditLogs]);
  ...
  return (
    <>
      ...
      <div className="flex w-full h-[calc(100vh-8rem)] min-h-0">
        <div className="flex-1 min-w-0">
          <DataGallery
            ...
            searchPlaceholder="Pesquisar métodos de aquisição..."
      ...
      {/* Modal de histórico com abas */}
      <HistoricoModalWithTabs
        open={historicoModalOpen}
        onOpenChange={setHistoricoModalOpen}
        title={`Histórico - ${selectedItemHistorico?.nome || ''}`}
        tabs={prepareHistoryTabs()}
      />
    </>
  );
};

export default ConfiguracaoMetodoAquisicao;
