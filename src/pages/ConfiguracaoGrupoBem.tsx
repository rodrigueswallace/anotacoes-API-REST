...
import { CadastrarGrupoBemModal } from '@/components/modals/CadastrarGrupoBemModal';
...
const ConfiguracaoGrupoBem = () => {
  ...
  const [selectedGrupoBemHistorico, setSelectedGrupoBemHistorico] = useState<any>(null);

  // Hook do Supabase para buscar dados
  const { 
    groups: data, 
    isLoading, 
    error,
  ...
  } = useAssetGroupAuditLog(
    selectedGrupoBemHistorico ? parseInt(selectedGrupoBemHistorico.id) : null
  );
  ...
  const breadcrumbPath = [
    ...
    { label: "Grupos de bem", current: true }
  ];
  ...
  const columns = [
    ...
    { key: 'codigo_contabil', title: 'Código contábil', sortable: true, width: 'auto' },
    { 
      key: 'depreciacao_aplicavel', 
      title: 'Depreciação aplicável', 
      sortable: true, 
      width: 'auto',
  ...
  const handleConfirmToggleStatus = async () => {
    ...
    try {
      await toggleAssetGroupStatus(item.id);
  ...
  const handleSaveGrupoBem = async (formData: any) => {
    ...
    try {
      if (isEditing && editingItem) {
        await updateAssetGroup({ id: editingItem.id, data: formData });
        completeEditFlow();
      } else {
      ...
      console.error('Erro ao salvar grupo de bem:', error);
      hideAllModals();
      throw error;
    }
  };
  ...
  // Função para adicionar novo grupo de bem
  const handleAddNew = () => {
    setEditingItem(null);
    setIsEditing(false);
    setModalOpen(true);
  ...
  const handleHistorico = (grupoBem: any) => {
    setSelectedGrupoBemHistorico(grupoBem);
  ...
  useEffect(() => {
    if (historicoModalOpen && selectedGrupoBemHistorico) {
      refetchAuditLogs();
    }
  }, [historicoModalOpen, selectedGrupoBemHistorico, refetchAuditLogs]);
  ...
  return (
    <>
      ...
      <div className="flex w-full h-[calc(100vh-8rem)] min-h-0">
        <div className="flex-1 min-w-0">
          <DataGallery
            ...
            searchPlaceholder="Pesquisar grupos de bem..."
      ...
      <StandardModalsWrapper
        ...
        onReactivateCancel={handleInactivateCancel}
      ...
      <HistoricoModal
        ...
        title={`Histórico - ${selectedGrupoBemHistorico?.nome || ''}`}
...
export default ConfiguracaoGrupoBem;
